...
pub fn parse_tail_position(bytes: [u8; U64SIZE * 3]) -> Option<u64> {
    let first = u64::from_le_bytes(
        (&bytes[..U64SIZE]).try_into().expect(SIZE_CHECKED)
    );
    let second = u64::from_le_bytes(
        (&bytes[U64SIZE..U64SIZE * 2]).try_into().expect(SIZE_CHECKED)
    );
    let third = u64::from_le_bytes(
        (&bytes[U64SIZE * 2..]).try_into().expect(SIZE_CHECKED)
    );
